...
export default function NotesLayout({ initialDisplayPosts = [], pagination }: Props) {
            ...
            return (
              <div className="" key={title}>
                <Link href={`/work/${slug}`} aria-label={`Link to ${title}`}>
                  <div className="flex flex-col mx-auto max-w-screen-xl rounded-[12px] border-2 transition-all duration-300 md:flex-row md:hover:bg-gray-200 md:dark:hover:bg-gray-800 cursor-pointer">
                    <Image
                      className="relative flex-shrink-0 rounded-t-[10px] md:w-1/2 md:rounded-none md:rounded-r-[10px]"
                      alt={title}
                      src={cover}
                      width={800}
                      height={600}
